...
fn analyse_data2( sequences: &[Vec<isize>] ) -> isize {
  let revs:Vec<Vec<isize>> = sequences.iter().map( |s| s.iter().rev().copied().collect::<Vec<_>>() ).collect();
  /*
  ...
  let sum = revs.iter().map( |s:&Vec<isize>| *s.last().unwrap() + extrapolate_seq(s) ).sum();
