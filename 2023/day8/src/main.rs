...
fn analyse_data2( dirs: &str, nodes: &HashMap<String, (String, String)> ) -> usize {
  ...
  for (a,b) in &nv {
    if *a == 0 && *b == 0 { panic!("uninitialised pos"); }
  }
  ...
  loop {
    ...
    let mut z_count = 0;
    for c in &cur_node {
      if !ends_z[*c] { all_z = false; }
      if ends_z[*c] { z_count += 1; }
    }
    if z_count > 0 { println!("{score} {z_count}");}

    if all_z {
      break;
    }

    for j in 0..cur_node.len() {
      let cn = &cur_node[j];
    ...
    i = (i+1)%dv.len();
    score += 1;
    if score % 1_000_000 == 0 { println!("progress: {score}"); }
  }
  score
}
