...
fn analyse_data( map: &mut TerrainMap<Pipe>, start: Point ) -> (usize, TerrainMap<bool>) {
  ...
  while let Some(v) = stack.pop() {
    ...
    for w in adjacent_edges(v, map.get(&v)) {
      ...
      if !dist.contains_key(&w) || *dist.get(&w).unwrap() != old_dist-1 { //don't add the distance for going A->B->A again
  ...
  let mut found = false;
  for p in all::<Pipe>() {
    if p == Pipe::Ground {continue}
    if p == Pipe::NESW {continue}
...
fn find_inside( map: &TerrainMap<Pipe>, loop_map: &TerrainMap<bool>) -> usize {
  ...
  for y in 0..map.dims.height {
    ...
    for x in 0..map.dims.width {
      ...
      if on_loop {
        ...
        if !inside_run {
          ...
          match run_start {
            ...
            Pipe::NE => {
              match cur_pipe {
                Pipe::SW => { outside = !outside; inside_run = false; }
                Pipe::NW => { inside_run = false; }
                _ => panic!("unexpected pipe in run {cur_pipe:?}")
              }
            }
            Pipe::SE => {
              match cur_pipe {
                Pipe::NW => { outside = !outside; inside_run = false; }
                Pipe::SW => { inside_run = false; }
                _ => panic!("unexpected pipe in run {cur_pipe:?}")
              }
            }
            _ => { panic!("unexpected start run of pipe {run_start:?}") }
          }
        }
      ...
      match outside {
        true => { out_count += 1; }
        false => { ins_count += 1; println!("Found inside at {p}"); }
      }
    }
  }
