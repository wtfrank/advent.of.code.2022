...
fn insert_range(range: Range<isize>, list: &mut LinkedList<Range<isize>>) {
  ...
  for r in list.iter_mut() {
    if range.end < r.start {
      ...
      //the ranges overlap (possibly more than once)
      let new_r = Range {
        start: isize::min(r.start, range.start),
...
fn create_range(sensor: Point, beacon: Point, row: isize) -> Option<Range<isize>> {
  ...
  Some(Range {
    start: sensor.x - (max_dist as isize - row_dist),
    end: sensor.x + 1 + (max_dist as isize - row_dist),
  })
}
...
fn load_scan(filename: &str, rows: usize) -> (Vec<LinkedList<Range<isize>>>, HashSet<Point>) {
  ...
  for _ in 0..rows + 1 {
    ranges.push(LinkedList::<Range<isize>>::new());
  }
